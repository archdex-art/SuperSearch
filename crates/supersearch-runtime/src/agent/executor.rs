...
use super::task_graph::{NodeId, TaskGraph, TaskNodeKind, TaskStatus};
    ...
    fn execute_node(&self, kind: &TaskNodeKind, timeout: Duration) -> StepResult {
        ...
        match kind {
            ...
            TaskNodeKind::ListRunningApps => {
                backend.list_running_apps("List running apps", timeout)
            }
...
mod tests {
    ...
    impl PlatformBackend for RecordingBackend {
        fn launch_app(&self, a: &str, _args: &[String], l: &str, _t: Duration) -> StepResult {
            self.ok(format!("launch:{a}"), l)
        }
        fn open_path(&self, p: &str, l: &str, _t: Duration) -> StepResult {
            self.ok(format!("open_path:{p}"), l)
        }
        fn open_url(&self, u: &str, l: &str, _t: Duration) -> StepResult {
            self.ok(format!("open_url:{u}"), l)
        }
        fn find_files(&self, q: &str, l: &str, _t: Duration) -> StepResult {
            self.ok(format!("find:{q}"), l)
        }
        fn clipboard_read(&self, l: &str, _t: Duration) -> StepResult {
            self.ok("clip_read".into(), l)
        }
        fn clipboard_write(&self, c: &str, l: &str, _t: Duration) -> StepResult {
            self.ok(format!("clip_write:{c}"), l)
        }
        fn list_running_apps(&self, l: &str, _t: Duration) -> StepResult {
            self.ok("list_apps".into(), l)
        }
        fn quit_app(&self, a: &str, l: &str, _t: Duration) -> StepResult {
            self.ok(format!("quit:{a}"), l)
        }
        fn switch_app(&self, a: &str, l: &str, _t: Duration) -> StepResult {
            self.ok(format!("switch:{a}"), l)
        }
        fn run_trusted_script(&self, s: &str, l: &str, _c: bool, _t: Duration) -> StepResult {
            self.ok(format!("script:{s}"), l)
        }
    }
    ...
    fn denied_action_never_reaches_the_backend() {
        ...
        assert!(
            ...
            "denied action reached the backend"
        );
    }
