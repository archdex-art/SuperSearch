      ...
      {open && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative w-full max-w-[640px] overflow-hidden rounded-2xl border border-white/15
                       bg-[hsla(228,16%,12%,0.72)] shadow-[0_32px_80px_-16px_rgba(0,0,0,0.6)]
                       ring-1 ring-inset ring-white/[0.06] backdrop-blur-2xl"
          >
            {/* Search input */}
            <div className="flex items-center gap-3 px-4 h-[58px] border-b border-white/[0.07]">
              <SearchIcon />
              <input
                ref={inputRef}
                value={query}
                ...
                className="h-full flex-1 bg-transparent text-[18px] font-normal text-white
                           caret-emerald-400 outline-none placeholder:text-white/35"
              />
            </div>
            ...
            <LayoutGroup>
              ...
              >
                {ordered.length === 0 ? (
                  <li className="flex flex-col items-center gap-1 px-4 py-14 text-center text-white/45">
                    {emptyState ?? (
                      <>
...
function Hint({ k, label }: { k: string; label: string }) {
  return (
    <span className="flex items-center gap-1.5">
      <kbd className="rounded-[5px] border border-white/15 bg-white/10 px-1.5 py-0.5 text-[11px] text-white/70">
        {k}
      </kbd>
